...
> = ({ state, descriptors, navigation, setRoute }) => {
  ...
  return (
    <View className="w-[90%] mx-auto flex flex-row items-center justify-between px-1">
      {state.routes.map((route, index) => {
        ...
        return (
          ...
          >
            ...
            <Text
              className={`font-bold ${isFocused ? "text-primary" : "text-secondary"
                }`}
            >
              {t(label)}
            </Text>
            <View
              className={`absolute bottom-0 h-[2px] w-full ${isFocused ? "bg-primary" : "bg-transparent"
                }`}
            />
          </TouchableOpacity>
        );
